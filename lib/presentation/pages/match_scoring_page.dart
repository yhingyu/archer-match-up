...
class _MatchScoringPageState extends State<MatchScoringPage>
  ...
  Match? _currentMatch; // Track current match state

  @override
  void initState() {
    super.initState();
    _tabController = TabController(length: 2, vsync: this);
    _currentMatch = widget.match; // Initialize with passed match
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text(_currentMatch?.title ?? widget.match.title),
        bottom: TabBar(
    ...
    return Column(
      children: [
        ...
        ScorecardWidget(
          ...
          match: _currentMatch ?? widget.match,
          isCurrentUser: true,
        ),
  ...
  Widget _buildIndividualArrowInput(ScoringProvider provider, scoreWithUser) {
    ...
    return ArrowInputWidget(
      maxArrows: (_currentMatch ?? widget.match).arrowsPerEnd,
      isEnabled: true,
      onArrowsSubmitted: (arrows) async {
        await provider.recordEnd(
          matchId: (_currentMatch ?? widget.match).matchId,
